...
void	start_animation(t_info *info)
{
	...
	while (y < 4)
	{
		...
		while (x < 170)
		{
			mlx_put_image_to_window(info->mlx, info->mlx_win, info->start_images[y].xpm_ptr, 300 + (120 * y), x);
			usleep(2 * 1400);
			x += 1;
			mlx_string_put(info->mlx, info->mlx_win, WIDTH / 2 - 50, HEIGHT / 2 + 50, color++, "click space to start");
		}
		y++;
	}
}
